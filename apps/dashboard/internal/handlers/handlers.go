...
var agentsTemplate = template.Must(template.New("agents").Funcs(funcMap).Parse(`<!DOCTYPE html>
<html>
<head>
...
var costsTemplate = template.Must(template.New("costs").Funcs(funcMap).Parse(`<!DOCTYPE html>
<html>
<head>
...
func (h *Handlers) AgentsPage(w http.ResponseWriter, r *http.Request) {
	...
	agentsTemplate.Execute(w, map[string]any{"Runs": runs})
}
...
func (h *Handlers) CostsPage(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "text/html")
	costsTemplate.Execute(w, map[string]any{
		"Summaries": summaries,
		"TotalCost": totalCost,
		"TotalRuns": totalRuns,
	})
